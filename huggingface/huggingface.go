...
// HuggingFace provider definition.
...
func (p *HuggingFace) Completion(ctx context.Context, options ...provider.Func) (string, error) {
	//////
...
func (p *HuggingFace) GetClient() any {
	return p.client
}
...
func New(
	...
	var _ provider.IProvider = (*HuggingFace)(nil)
...
// NewDefault creates a new HuggingFace provider with default values.
func NewDefault(options ...provider.ClientFunc) (*HuggingFace, error) {
	opts := []provider.ClientFunc{
		provider.WithEndpoint(config.Get().HuggingFaceEndpoint),
		provider.WithToken(config.Get().HuggingFaceToken),
	}
